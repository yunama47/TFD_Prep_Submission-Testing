...
X, Y = sbds.loadLinRegDataset('C')
(train_images_c2, train_labels_c2), (test_images_c2, testing_labels_c2) = sbds.loadImagesData('MNIST')
(train_images_c3, train_labels_c3), (test_images_c3, testing_labels_c3) = sbds.loadImagesData('cats_and_dogs')

training_padded, training_labels, test_padded, test_labels = sbds.loadTextsData('sarcasm')
# train_set, test_set = sbds.loadDailyMinTemp()
...
class TestSubmissionC(unittest.TestCase):
    ...
    def testC3(self): # cats dogs
        global train_images_c3, train_labels_c3, test_images_c3, testing_labels_c3 
        model_C3 = keras.models.load_model(os.path.join(MODEL_DIR,'model_C3.h5'))
        _, result_train = model_C3.evaluate(train_images_c3, train_labels_c3, verbose=0)
        _, result_test = model_C3.evaluate(test_images_c3, testing_labels_c3, verbose=0)
